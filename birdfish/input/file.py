...
class EventReader(threading.Thread):
    ...
    def run(self):
        ...
        # TODO self.run_started and self.started seem redundant and may not be
        # needed?
        self.run_started = time.time()
        self.seek_start()
        while not self._stopevent.is_set():
            # TODO do I need an attribute here to only run full loops - or
            # always respect duration?
                ...
                try:
                    ...
                    if self.looping:
                        if self.looping_duration:
                            if (time.time() >
                                    self.run_started + self.looping_duration):
                                self._stopevent.set()
                                continue
                        self.init_reader()
                        self.started = 0
            ...
            if time_delta >= event_delta:
                if self.pending_event[0][0].lower() == 't':
                    ...
                    lightobj = self.show.get_named_element(
                            self.pending_event[1])
            ...
            if not self.pending_message:
                ...
                if l:
                    ...
                    if self.looping:
                        ...
                        self._stopevent.set()
                        continue
            tracktime = (((time.time() - self.started) * 1000)
                    + self.start_offset)
...
class BasicScheduledPlayer(threading.Thread):
    """
    A simple manager of trigger file playing will need something more put
    together later @@
    """
    def __init__(self, *arg, **kwargs):
        ...
        # (tm_year=2010, tm_mon=12, tm_mday=3, tm_hour=4, tm_min=51, tm_sec=4,
        # tm_wday=4, tm_yday=337, tm_isdst=0)
